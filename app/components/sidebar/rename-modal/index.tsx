...
const RenameModal: FC<IRenameModalProps> = ({
  ...
  return (
    ...
    >
      <div className={'mt-6 font-medium text-sm leading-[21px] text-gray-900'}>{t('app.chat.conversationName')}</div>
      <input className={'mt-2 w-full rounded-lg h-10 box-border px-3 text-sm leading-10 bg-gray-100'}
        value={tempName}
        onChange={e => setTempName(e.target.value)}
        placeholder={t('app.chat.conversationNamePlaceholder') || ''}
      />

      <div className='mt-10 flex justify-end'>
        <Button className='mr-2 flex-shrink-0 !text-[#101A3C] !hover:bg-[#101A3C] !hover:text-white' onClick={onClose}>{t('app.chat.operation.cancel')}</Button>
